...
void notEmptyFunction() {
    ...
    auto longLineLambda
        = [](int a,
             int b,
             int casfdasfasdfhasdkjfhaskdjfhkasdhfkashdfjkhaskfjhas) {
              print(a);
          };
    longLineLambda();
}
    ...
    namespace B {
        namespace C {
            namespace D {
                ...
                int
                veryLongFunction(int              anotherArgument,
                                 int              beforeCallMe,
                                 bool             isThisOkForYouMan,
                ...
                // clang should restore comment if missing and remove everything
                // afterwords
            }  // namespace D
        }      // namespace C
    }          // namespace B
...
namespace A {
    namespace C {
        int
        veryLongFunctionAbcAASKJHGAKLSFNCJKASDNCUIABSALSKJDNKAJSHDKJASHDJHKJASDHKJAHBSD(
            int              anotherArgument,
            int              beforeCallMe,
            bool             isThisOkForYouMan,
            std::vector<int> unknownValueForThisParameter) {
...
int main() {
    ...
    do { std::random_shuffle(list, list + 9); } while (is_sorted(list));
    ...
    dialog->addButton(i18n::tr("$OkId"),
                      "common/green_frame.9.png",
                      [this](PandemicDialog* dialog) {
                          std::cout << "Congratulations, you sorted the list.\n"
                                    << "You needed " << score << " reversals."
                                    << std::endl;
                          inherited::onBack();
                      });
    dialog->addButton(i18n::tr("$CancelId"),
                      "common/red_frame.9.png",
...
template<class T> struct FooT {
    ...
    template<> int foo<2>() { return Min<>(1, 5); }
...
// comment
EmptyStruct emptyStruct = EmptyStruct {};  // 1 space after ';'' otherwise: ;//1
                                           // space after...
